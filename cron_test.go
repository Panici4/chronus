...
func TestCronWithOffset(t *testing.T) {
	chronus := NewChronus("aa", time.Duration(0))
	cron := chronus.NewCron()
	cancel := make(chan struct{})
	id, err := cron.AddFunc("at 2025-03-03 00:00:00", func() {
		fmt.Println("test")
		cancel <- struct{}{}
	})
	fmt.Println(err)
	cron.Start()
	setTime, _ := time.ParseInLocation("20060102 15:04:05", "20250302 23:59:50", time.Local)
	chronus.SetOffset(setTime.Sub(time.Now()))
	start := time.Now()
	select {
	...
	for _, e := range cron.scheduled {
		if id == e.ID {
			t.Fatal("not remove once")
		}
	}
...
func TestLoad(t *testing.T) {
	NewChronus("aa", time.Duration(0))
	fmt.Println(GetByID("aa"))
	fmt.Println(GetByID("bb"))
}
